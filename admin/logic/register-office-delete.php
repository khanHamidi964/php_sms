    ...
    if ($_SESSION['role'] == 'Admin') {
        ...
        if (isset($_GET['r_user_id'])) {
            ...
            if (is_numeric($r_user_id)) {
                ...
                $query = "DELETE FROM registerer_office WHERE r_user_id = ? LIMIT 1";
                $query_stmt = mysqli_prepare($connection , $query);
                mysqli_stmt_bind_param($query_stmt , 'i' , $r_user_id);
                
                if (mysqli_stmt_execute($query_stmt)) {

                    $mess = "Register Office User successfully deleted!";
                ...
                $mess = "register-office Id is not a number ";
                header("location: ../register-office.php?error=$mess");
                exit;
            }
        } else {
